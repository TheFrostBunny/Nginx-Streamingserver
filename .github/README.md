...
## Konvertere HLS-strøm til video (MP4)

Når en stream er ferdig, kan du konvertere HLS-strømmen til en MP4-videofil for arkiv:

1. Finn stream-ID-en (samme som filnavnet uten .m3u8 i /hls-mappen).
2. Kjør følgende kommando for å konvertere til video:

```bash
curl -X POST http://localhost:3000/api/convert/<streamId>
```

Dette vil lagre en MP4-fil i `public/uploads`, og videoen blir synlig i videoarkivet.

---

## Automatisk oppdatering av strømmer

Dashboardet oppdaterer nå kun listen over aktive strømmer uten å reloade hele siden. Nye og avsluttede strømmer håndteres dynamisk, og eksisterende strømmer fortsetter å spille selv om andre starter eller stopper.
